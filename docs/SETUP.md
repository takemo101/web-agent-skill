...
```bash
# スキル一覧に web-test が表示されることを確認
taskp list

# テスト実行（example.com で簡易テスト）
taskp run web-test
```
...
テスト手順: ページのタイトルが "Example Domain" であることを確認する
期待結果: （空欄）
ヘッドレスモード: Yes
```

成功すれば `midscene_run/report/` にHTMLレポートが生成される。
...
taskp run web-test --model anthropic/claude-sonnet-4-20250514
```
